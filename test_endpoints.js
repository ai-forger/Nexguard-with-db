...
async function testEndpoints() {
    try {
        ...
        const mintRes = await axios.post(`${API_URL}/simulate/mint`, { name: "TestToken", symbol: "TST" });
        const tokenId = mintRes.data.tokenId;
        console.log("   Minted:", tokenId);

        console.log("2. Testing /publish/:tokenId...");
        const pubRes = await axios.post(`${API_URL}/publish/${tokenId}`);
        ...
        const voteRes = await axios.post(`${API_URL}/vote`, { tokenId: tokenId, vote: "agree" });
        console.log("   Vote Response:", voteRes.data);

        console.log("5. Testing /trade...");
        const tradeRes = await axios.post(`${API_URL}/trade`, { tokenId: tokenId, type: "buy", amount: 100 });
